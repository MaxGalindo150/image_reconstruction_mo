...
def generate_measurements(signal: SimpleNamespace, model: SimpleNamespace) -> SimpleNamespace: #sinal
    ...
    input_signal = np.concatenate((signal.u[:, np.newaxis], np.zeros((model.Ny - signal.Ni, 1))))

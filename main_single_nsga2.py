...
def main():
    ...
    img_dir = "img/nsga2_pro/single/nsga2"

    # Configurar el problema
    problem, PROBE, MODEL, SIGNAL, d_lcurve, lambda_lcurve = setup_problem(example=1, sigma=0.1)

    # Ejecutar la optimización
    res, archive, hv_values = run_optimization_nsga2(problem, ref_point, archive_file, img_dir)
    ...
    output_csv_file = "solutions_comparison_nsga2.csv"
    ...
    plot_hypervolume(hv_values, img_dir, "NSGA-II")
    plot_pareto_front(
        res.F, "NSGA-II",archive=archive, img_dir=img_dir,
        lcurv_sol=l_curve_sol_values, best_sol=best_solution_values
    )
    plot_best_solution(img_dir, PROBE, mu_est_nsga2, mu_est_l_curve, "NSGA-II")
    
    # Graficar análisis adicional del frente de Pareto
    # Visualización con clustering
    visualize_pareto_front(objectives, img_dir, "NSGA-II", clusters=3)

    # Proyecciones 2D
    visualize_pareto_2d_projections(objectives, img_dir, "NSGA-II")

    
    visualize_pareto_with_weights(objectives, weights, img_dir, "NSGA-II")
